# -*- coding: utf-8 -*-
"""Копия блокнота "neurobot_reworked.ipynb"

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1o1ikBRoEswQc07mDrtjSdK7vegCeVzqf
"""
import time
import os
...
import statistics
import imutils
from math import sqrt, atan2, degrees, atan, tan
...
cross2squareK: float = 43 / 34  # 20 / 15
...
img = cv2.detailEnhance(img, sigma_s=300, sigma_r=10)
img = heq(img)
img = bright_contrast(image, -20, 20)  # brightness and contrast
...
def filter_list(lst, places=5):
    ...
    med_h = statistics.median([d['w'] for d in cnts_info])
    med_w = statistics.median([d['h'] for d in cnts_info])
    ...
    for d in lst:
        if d['w'] not in rw and d['h'] not in rw:
            result.append(d)
        ...
        if set(cntRx).issubset(rX) and set(cntRy).issubset(rY):
            ...
            if len(cnts_info) > 1:
                ...
                for other in cnts_info:
                    ...
                    print(dist_to_other * 2 / max(cnt['w'], cnt['h']))
                    if dist_to_other * 2 / max(cnt['w'], cnt['h']) > 6 and dist_to_other != 0:
                        flag = True
                    elif dist_to_other != 0:
                        flag = False
                        temp_perfect = cnt
                        break
                    else:
...
if len(perfects) == 0 or len(perfects[-1]) == 0:
...
img = (img[
       center_y - round(plane_h / 2): center_y + round(plane_h / 2),
